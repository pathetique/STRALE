...
static int backport_rawsock_sendmsg(struct kiocb *iocb, struct socket *sock, struct msghdr *msg, size_t len)
{
...
#if LINUX_VERSION_CODE < KERNEL_VERSION(4,1,0)
static int backport_rawsock_recvmsg(struct kiocb *iocb, struct socket *sock, struct msghdr *msg, size_t len, int flags)
{
